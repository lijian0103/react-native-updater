...
import cn.cnlee.commons.updater.utils.ColorUtil;
import cn.cnlee.commons.updater.utils.DrawableUtil;
import cn.cnlee.commons.updater.view.NumberProgressBar;
...
//        getDialog().getWindow().addFlags(WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL);
        ...
        getDialog().setOnKeyListener(new DialogInterface.OnKeyListener() {
            ...
            public boolean onKey(DialogInterface dialog, int keyCode, KeyEvent event) {
                if (keyCode == KeyEvent.KEYCODE_BACK) {
                    //禁用
                    if (mUpdateApp != null && mUpdateApp.isForced()) {
                        //返回桌面
    ...
    private void initData() {
        ...
        if (mUpdateApp != null) {
            //弹出对话框
            final String dialogTitle = mUpdateApp.getUpdateDialogTitle();
            final String newVersion = mUpdateApp.getVersionName();
            final String targetSize = mUpdateApp.getSize();
            final String updateLog = mUpdateApp.getUpdateContent();
